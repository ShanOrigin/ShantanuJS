...
export function getAllElementsCanvasMethod() {
  ...
  testEnv.env({
    ...
    run(ctx) {
      testEnv.shTest({
        ...
        // --------------------------------------------------
        testInfo: {
          description: `Canvas .getAllElements() method testing`,
          ...
          validators: {
            id: {
              ...
              validate(shape, expected) {
                delete ctx.shapes.canvas;

                const shapes = Object.values(ctx.shapes);

                const allElements = ctx.canvas.getAllElements();
                const allPresent = shapes.every((shape) => {
                  return allElements.includes(shape as Shape);
                });
