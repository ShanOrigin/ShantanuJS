...
export class Animation<T extends GShpesTages> {
  ...
  public animate(
    ...
    advProp && typeof advProp === 'object' && deepMerge(this.#advInfo, advProp); // merge user Advance props with default one
      ...
      if (cp) {
        ...
        if (
          ...
          (typeof pivot.skewPivot === 'string' &&
            (pivot.skewPivot = pivotSetter(pivot.skewPivot as anchors, OBB))) ||
            (pivot.skewPivot = [0, 0]);
        }
      }
    }
