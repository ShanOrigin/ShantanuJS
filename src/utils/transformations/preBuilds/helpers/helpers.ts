...
/**
 * Ensures a valid transformation matrix by converting or validating inputs.
 *
 * Purpose:
 * Accepts either a 2D array (`number[][]`) or a `DOMMatrix`. If the input is a 2D array,
 * it converts it into a `DOMMatrix` with proper column-major mapping. Then it validates that
 * all required numeric matrix components exist and are valid numbers.
 *
 * Parameters:
 * @param tmat - Either:
 *   - A 2D array representing the transformation matrix (2x3 or 3x3), or
 *   - A `DOMMatrix` instance.
 *
 * Returns:
 * - A `DOMMatrix` instance representing the same transformation if input was a 2D array.
 * - Returns the original `DOMMatrix` if input was already a valid matrix.
 *
 * Dependencies:
 * - Relies on the browser's `DOMMatrix` API. Cannot run in environments without `DOMMatrix`.
 */

export function setTMatrix(tmat: number[][] | DOMMatrix): void | DOMMatrix {
  // If it's not a DOMMatrix, convert number[][] → DOMMatrix
  if (!(tmat instanceof DOMMatrix)) {
    const tlen = tmat.length;
    const row1len = tmat?.[0]?.length ?? 0;
    const row2len = tmat?.[1]?.length ?? 0;
    const row3len = tmat?.[2]?.length ?? 0;
    const same = row1len === 3 && row2len === 3;
    const isValidT =
      (tlen === 2 && same) || (tlen === 3 && same && row3len === 3);
    console.log(!isValidT);
    if (!isValidT) {
    ...
    const [a, c, e] = tmat[0];
    const [b, d, f] = tmat[1];
    const [g, h, i] = tmat[2] ?? [0, 0, 1];

    // Construct DOMMatrix (row-major to column-major mapping)
    tmat = new DOMMatrix([
      a,
      b,
      0,
      ...
      h,
      i,
      0, // Column 3 (perspective)
      e,
      f,
      0,
      1 // Column 4 (translate + last row)
    ]);
  }
  ...
  for (const k in v) {
    ...
    const va = (v as any)[k];
    console.log(va, typeof va !== 'number', Number.isNaN(va));
    if (typeof va !== 'number' || Number.isNaN(va)) {
      throw new Error('Invalid Transformation Matrix');
    }
  }
