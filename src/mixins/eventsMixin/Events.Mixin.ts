 ...
 *
 * This mixin equips a class with a minimal event system:
 * - `on()`   → register event listeners
 * - `off()`  → remove listeners
 * - `emit()` → dispatch events to listeners
 *
 * It does NOT modify the nature of the underlying class — it simply
 * layers event functionality on top of it. The original generics of the
 * base class are fully preserved.
 *
 * This allows the engine to keep clean separation:
 * - Core rendering logic stays in the base class
 * - Event behavior is optional and composable
 *
